...
}), "Private non-secure context can fetch local subresource.");
...
promise_test(t => fetchTest(t, {
  source: { treatAsPublicAddress: true },
  expected: kFetchTestResult.failure,
}), "Treat-as-public-address non-secure context can fetch local subresource.");

promise_test(t => fetchTest(t, {
  source: { treatAsPublicAddress: true },
  target: { port: kPorts.httpPrivate },
  expected: kFetchTestResult.failure,
}), "Treat-as-public-address non-secure context can fetch private subresource.");
...
promise_test(t => fetchTest(t, {
  ...
  expected: kFetchTestResult.failure,
}), "Private HTTPS non-secure context can fetch local subresource.");
